...
function main_pars($num_m){
    ...
    $penalty_kill_t1_one = $file_phpQuery->find('.b-data_row:nth-child(7) .dataTables_wrapper:nth-child(4) table.dataTable.stripe.compact.row-border.hl.no-footer.rc.m-table_small:nth-child(1) tbody tr:nth-child(1) td:nth-child(7)')->text();
    $penalty_kill_t1_two = $file_phpQuery->find('.b-data_row:nth-child(7) .dataTables_wrapper:nth-child(4) table.dataTable.stripe.compact.row-border.hl.no-footer.rc.m-table_small:nth-child(1) tbody tr:nth-child(1)  td:nth-child(8)')->text();
    $p['penalty_kill_t1'] = $penalty_kill_t1_one.'/'.$penalty_kill_t1_two;
    $penalty_kill_t2_one = $file_phpQuery->find('.b-data_row:nth-child(7) .dataTables_wrapper:nth-child(4) table.dataTable.stripe.compact.row-border.hl.no-footer.rc.m-table_small:nth-child(1) tbody tr:nth-child(2) td:nth-child(7)')->text();
    $penalty_kill_t2_two = $file_phpQuery->find('.b-data_row:nth-child(7) .dataTables_wrapper:nth-child(4) table.dataTable.stripe.compact.row-border.hl.no-footer.rc.m-table_small:nth-child(1) tbody tr:nth-child(2)  td:nth-child(8)')->text();
    $p['penalty_kill_t2'] = $penalty_kill_t2_one.'/'.$penalty_kill_t2_two;
    //$num_row=
    // силовые приемы
    $p['body_check_t1'] = trim($file_phpQuery->find('.b-data_row:nth-child(9) table tbody tr:nth-child(5) td:nth-child(3)')->text());
    $p['body_check_t2'] = trim($file_phpQuery->find('.b-data_row:nth-child(9) table tbody tr:nth-child(5) td:nth-child(6)')->text());
    // блокированные броски
    $p['block_throw_t1'] = trim($file_phpQuery->find('.b-data_row:nth-child(9) table tbody tr:nth-child(5) td:nth-child(2)')->text());
    $p['block_throw_t2'] = trim($file_phpQuery->find('.b-data_row:nth-child(9) table tbody tr:nth-child(5) td:nth-child(5)')->text());
    echo '<br>силовые приемы '.$p['block_throw_t1'];
    echo '<br>силовые приемы '.$p['block_throw_t2'];
    // время в атаке
    $p['atack_time_t1'] = trim($file_phpQuery->find('.b-data_row:nth-child(9) table tbody tr:nth-child(5) td:nth-child(4)')->text());
    $p['atack_time_t2'] = trim($file_phpQuery->find('.b-data_row:nth-child(9) table tbody tr:nth-child(5) td:nth-child(7)')->text());
...
function imageTemplate(){
    ...
    //$image = imagecreatefrompng('protocol_OT+.png');protocol_B.png
    $image = imagecreatefrompng('protocol_B.png');
    // установка шрифтов
    ...
    switch($p['name_t1']){
        ...
        case 'СКА':imagettftext($image, 65, 0, 450, 265, $white, $font_BigNoodle, $p['name_t1']); break;
        case 'Барыс':imagettftext($image, 65, 0, 400, 265, $white, $font_BigNoodle, $p['name_t1']); break;
        default:imagettftext($image, 65, 0, 275, 265, $white, $font_BigNoodle, $p['name_t1']);
    }
    imagettftext($image, 65, 0, 715, 265, $white, $font_BigNoodle, $p['name_t2']);
    // втавка общего счета
    imagettftext($image, 80, 0, 500, 155, $white, $font, $p['total_score_t1']);
    imagettftext($image, 80, 0, 690, 155, $white, $font, $p['total_score_t2']);
    //imagettftext($image, 40, 0, 600, 145, $bl_blue, $font, $p['match_len']);// OT
    imagettftext($image, 40, 0, 620, 145, $bl_blue, $font, $p['match_len']);// OT
    ...
    imagettftext($image, 45, 0, 505, 325, $white, $font_BigNoodle, $p['period_score']);
    // втавка заброшенных шайб 1 периода
    $period_1=0;
    foreach ($p['goals_1'] as $val){
        imagettftext($image, 20, 0, 95, 420+$period_1, $white, $font_goals, $val['score'].' - '.$val['time_m'].'.'.$val['time_s'].' '.$val['family']);
        if ($val['assist_1_family'] != ''){ // для проверки наличия ассистентов
        imagettftext($image, 15, 0, 95, 440+$period_1, $silver, $font_goals, '('.$val['assist_1_family'].', '.$val['assist_2_family'].')');
        }
    ...
    foreach ($p['goals_2'] as $val){
        imagettftext($image, 20, 0, 485, 420+$period_2, $white, $font_goals, $val['score'].' - '.$val['time_m'].'.'.$val['time_s'].' '.$val['family']);
        if ($val['assist_1_family'] != ''){ // для проверки наличия ассистентов
        imagettftext($image, 15, 0, 485, 440+$period_2, $silver, $font_goals, '('.$val['assist_1_family'].', '.$val['assist_2_family'].')');
        }
    ...
    foreach ($p['goals_3'] as $val){
        imagettftext($image, 20, 0, 880, 420+$period_3, $white, $font_goals, $val['score'].' - '.$val['time_m'].'.'.$val['time_s'].' '.$val['family']);
        if ($val['assist_1_family'] != ''){ // для проверки наличия ассистентов
        imagettftext($image, 15, 0, 880, 440+$period_3, $silver, $font_goals, '('.$val['assist_1_family'].', '.$val['assist_2_family'].')');
        }
        $period_3 =$period_3+55;
    }
    // втавка заброшенных шайб в ОТ
//    imagettftext($image, 20, 0, 545, 680, $white, $font_goals, $p['goals_OT'][0]['score'].' - '.$p['goals_OT'][0]['time_m'].'.'.$p['goals_OT'][0]['time_s'].' '.$p['goals_OT'][0]['family']);
//    if ($p['goals_OT'][0]['assist_1_family'] != ''){ // для проверки наличия ассистентов
//        imagettftext($image, 15, 0, 545, 700, $silver, $font_goals, '('.$p['goals_OT'][0]['assist_1_family'].', '.$p['goals_OT'][0]['assist_2_family'].')');
//    }
    // втавка буллиты
    imagettftext($image, 20, 0, 545, 687, $white, $font_goals, $p['goals_B'][0]['score'].' - '.$p['goals_B'][0]['time_m'].'.'.$p['goals_B'][0]['time_s'].' '.$p['goals_B'][0]['family']);
    if ($p['goals_B'][0]['assist_1_family'] != ''){ // для проверки наличия ассистентов
        imagettftext($image, 15, 0, 545, 700, $silver, $font_goals, '('.$p['goals_B'][0]['assist_1_family'].', '.$p['goals_B'][0]['assist_2_family'].')');
    }
