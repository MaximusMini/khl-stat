...
$image = imagecreatefrompng('../template_img/temp_p2.png');
...
// дата
    imagettftext($image, 13, 0, 305, 127, $green , $font, $date_match);
// 1-пропущенные шайбы (таблица=table_conf cтолбец=miss_puck)
    imagettftext($image, $s_f, 0, 110, 380, $grey, $font, $row_team_1['miss_puck']);
    imagettftext($image, $s_f, 0, 550, 380, $grey, $font, $row_team_2['miss_puck']);
// 2-штрафное время (таблица=stat_penalty столбец=penalty_time)
    imagettftext($image, $s_f, 0, 110, 427, $grey, $font, $row_stat_penalty_team_1['penalty_time']);
    imagettftext($image, $s_f, 0, 550, 427, $grey, $font, $row_stat_penalty_team_2['penalty_time']);
// 3-броски по воротам за матч (таблица=stat_throw столбец=total_throw_average)
    imagettftext($image, $s_f, 0, 110, 473, $grey, $font, $row_stat_throw_team_1['total_throw_average']);
    imagettftext($image, $s_f, 0, 550, 473, $grey, $font, $row_stat_throw_team_2['total_throw_average']);
// 4-реализация бросков (таблица=stat_trow_percent столбец=throw_perc_total)
    imagettftext($image, $s_f, 0, 110, 522, $grey, $font, $row_stat_trow_percent_team_1['throw_perc_total']);
    //imagettftext($image, 24, 0, 145, 524, $grey, $font_name_team,'%');
    imagettftext($image, $s_f, 0, 550, 522, $grey, $font, $row_stat_trow_percent_team_2['throw_perc_total']);
...
// 5-броски соперника за матч (таблица=stat_throw_rival столбец=throw_rival_average)
    imagettftext($image, $s_f, 0, 110, 567, $grey, $font, $row_stat_throw_rival_team_1['throw_rival_average']);
    imagettftext($image, $s_f, 0, 550, 567, $grey, $font, $row_stat_throw_rival_team_2['throw_rival_average']);
// 6-реализация бросков соперником (таблица=stat_trow_percent столбец=throw_rival_perc_total)
    imagettftext($image, $s_f, 0, 110, 614, $grey, $font, $row_stat_trow_percent_team_1['throw_rival_perc_total']);
    //imagettftext($image, 24, 0, 145, 617, $grey, $font_name_team,'%');
    imagettftext($image, $s_f, 0, 550, 614, $grey, $font, $row_stat_trow_percent_team_2['throw_rival_perc_total']);
    //imagettftext($image, 24, 0, 596, 617, $grey, $font_name_team,'%');
// 7-вбрасывания (таблица=stat_faceoff столбец=faceoff_total)
    imagettftext($image, $s_f, 0, 110, 663, $grey, $font, $row_stat_faceoff_team_1['faceoff_total']);
    imagettftext($image, $s_f, 0, 550, 663, $grey, $font, $row_stat_faceoff_team_2['faceoff_total']);
// 8-выигранные вбрасывания  (таблица=stat_faceoff столбец=faceoff_perc_wins_total)
    imagettftext($image, $s_f, 0, 110, 708, $grey, $font, $row_stat_faceoff_team_1['faceoff_perc_wins_total']);
    //imagettftext($image, 20, 0, 150, 712, $grey, $font_name_team,'%');
    imagettftext($image, $s_f, 0, 550, 708, $grey, $font, $row_stat_faceoff_team_2['faceoff_perc_wins_total']);
...
// 9-реализация большинства (таблица=stat_pow_play_pow_kill столбец=perc_power_play)
    imagettftext($image, $s_f, 0, 110, 755, $grey, $font, $row_stat_pow_play_pow_kill_team_1['perc_power_play']);
    imagettftext($image, $s_f, 0, 550, 755, $grey, $font, $row_stat_pow_play_pow_kill_team_2['perc_power_play']);
// 10-реализация меньшинства соперником (таблица=stat_pow_play_pow_kill столбец=perc_power_kill)
    imagettftext($image, $s_f, 0, 110, 802, $grey, $font, $row_stat_pow_play_pow_kill_team_1['perc_power_kill']);
    imagettftext($image, $s_f, 0, 550, 802, $grey, $font, $row_stat_pow_play_pow_kill_team_2['perc_power_kill']);
    
    
imagepng($image,'../template_img/new/match_'.$number_match.'_temp-p2.png',9);
