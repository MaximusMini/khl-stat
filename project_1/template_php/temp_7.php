...
<head>
    ...
    <title>Шаблон 7 (штрафное время) </title>
</head>
...
$id_connect_DB = new mysqli('localhost', 'root', '07011989', 'db_preview');
    if($id_connect_DB){
        // формируем и выполняем запрос к таблице table_conf 
            $query_team_1 = 'SELECT * FROM stat_penalty WHERE id_team='.$id_team_1;
            $data_team_1 = $id_connect_DB->query($query_team_1);
            $query_team_2 = 'SELECT * FROM stat_penalty WHERE id_team='.$id_team_2;
...
$image = imagecreatefrompng('../template_img/temp_6.png');
...
// 1-штрафное время (таблица=stat_puck cтолбец=throw_puck)
    imagettftext($image, $s_f, 0, 110, 380, $grey, $font, $row_team_1['penalty_time']);
    imagettftext($image, $s_f, 0, 555, 380, $grey, $font, $row_team_2['penalty_time']);
// 2-штрафное время дома (таблица=stat_puck cтолбец=clear_loss_home)
    imagettftext($image, $s_f, 0, 110, 430, $grey, $font, $row_team_1['penalty_time_home']);
    imagettftext($image, $s_f, 0, 555, 430, $grey, $font, $row_team_2['penalty_time_home']);
// 3-штрафное время в гостях (таблица=stat_puck cтолбец=clear_loss_guest)
    imagettftext($image, $s_f, 0, 110, 476, $grey, $font, $row_team_1['penalty_time_guest']);
    imagettftext($image, $s_f, 0, 555, 476, $grey, $font, $row_team_2['penalty_time_guest']);
// 4-в среднем за матч (таблица=stat_puck cтолбец=ot_loss)
    imagettftext($image, $s_f, 0, 110, 523, $grey, $font, $row_team_1['penalty_time_average']);
    imagettftext($image, $s_f, 0, 555, 523, $grey, $font, $row_team_2['penalty_time_average']);
// 5-в среднем за матч дома (таблица=stat_puck cтолбец=ot_loss_home)
    imagettftext($image, $s_f, 0, 110, 570, $grey, $font, $row_team_1['penalty_time_average_home']);
    imagettftext($image, $s_f, 0, 555, 570, $grey, $font, $row_team_2['penalty_time_average_home']);
// 6-в среднем за матч в гостях (таблица=stat_puck cтолбец=ot_loss_guest)
    imagettftext($image, $s_f, 0, 110, 617, $grey, $font, $row_team_1['penalty_time_average_guest']);
    imagettftext($image, $s_f, 0, 555, 617, $grey, $font, $row_team_2['penalty_time_average_guest']);
    
imagepng($image,'../template_img/new/match_'.$number_match.'_temp_6.png',9);
