...
<head>
    ...
    <title>Шаблон 4 (поражения) </title>
</head>
...
$id_connect_DB = new mysqli('localhost', 'root', '', 'db_preview');
    if($id_connect_DB){
        // формируем и выполняем запрос к таблице table_conf 
            $query_team_1 = 'SELECT * FROM stat_loss WHERE id_team='.$id_team_1;
            $data_team_1 = $id_connect_DB->query($query_team_1);
            $query_team_2 = 'SELECT * FROM stat_loss WHERE id_team='.$id_team_2;
...
$image = imagecreatefrompng('../template_img/temp_3.png');
...
// 1-победы (таблица=stat_loss cтолбец=clear_loss)
    imagettftext($image, $s_f, 0, 130, 380, $grey, $font, $row_team_1['clear_loss']);
    imagettftext($image, $s_f, 0, 570, 380, $grey, $font, $row_team_2['clear_loss']);
// 2-победы дома (таблица=stat_loss cтолбец=clear_loss_home)
    imagettftext($image, $s_f, 0, 130, 430, $grey, $font, $row_team_1['clear_loss_home']);
    imagettftext($image, $s_f, 0, 570, 430, $grey, $font, $row_team_2['clear_loss_home']);
// 3-победы в гостях (таблица=stat_loss cтолбец=clear_loss_guest)
    imagettftext($image, $s_f, 0, 130, 476, $grey, $font, $row_team_1['clear_loss_guest']);
    imagettftext($image, $s_f, 0, 570, 476, $grey, $font, $row_team_2['clear_loss_guest']);
// 4-победы в ОТ (таблица=stat_loss cтолбец=ot_loss)
    imagettftext($image, $s_f, 0, 130, 523, $grey, $font, $row_team_1['ot_loss']);
    imagettftext($image, $s_f, 0, 570, 523, $grey, $font, $row_team_2['ot_loss']);
// 5-победы в ОТ дома (таблица=stat_loss cтолбец=ot_loss_home)
    imagettftext($image, $s_f, 0, 130, 570, $grey, $font, $row_team_1['ot_loss_home']);
    imagettftext($image, $s_f, 0, 570, 570, $grey, $font, $row_team_2['ot_loss_home']);
// 6-победы в ОТ в гостях (таблица=stat_loss cтолбец=ot_loss_guest)
    imagettftext($image, $s_f, 0, 130, 617, $grey, $font, $row_team_1['ot_loss_guest']);
    imagettftext($image, $s_f, 0, 570, 617, $grey, $font, $row_team_2['ot_loss_guest']);
// 7-победы по БТ (таблица=stat_loss cтолбец=b_loss)
    imagettftext($image, $s_f, 0, 130, 665, $grey, $font, $row_team_1['b_loss']);
    imagettftext($image, $s_f, 0, 570, 665, $grey, $font, $row_team_2['b_loss']);
// 8-победы по БТ дома  (таблица=stat_loss cтолбец=b_loss_home)
    imagettftext($image, $s_f, 0, 130, 712, $grey, $font, $row_team_1['b_loss_home']);
    imagettftext($image, $s_f, 0, 570, 712, $grey, $font, $row_team_2['b_loss_home']);
// 9-победы по БТ в гостях (таблица=stat_loss cтолбец=b_loss_guest)
    imagettftext($image, $s_f, 0, 130, 760, $grey, $font, $row_team_1['b_loss_guest']);
    imagettftext($image, $s_f, 0, 570, 760, $grey, $font, $row_team_2['b_loss_guest']);
    
imagepng($image,'../template_img/new/match_'.$number_match.'_temp_3.png',9);
