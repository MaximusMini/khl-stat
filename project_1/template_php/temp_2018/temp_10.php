...
<head>
    ...
    <title>Шаблон 10 (вбрасывания) </title>
</head>
...
$id_connect_DB = new mysqli('localhost', 'root', '', 'db_preview');
    if($id_connect_DB){
        // формируем и выполняем запрос к таблице table_conf 
            ...
            $query_team_2 = 'SELECT * FROM stat_faceoff WHERE id_team='.$id_team_2;
...
$image = imagecreatefrompng('../template_img/temp_9.png');
...
// 1-количество вбрасываний (таблица=stat_faceoff cтолбец=faceoff_total)
    imagettftext($image, $s_f, 0, 115, 380, $grey, $font, $row_team_1['faceoff_total']);
    imagettftext($image, $s_f, 0, 560, 380, $grey, $font, $row_team_2['faceoff_total']);
// 2-вбрасывания дома (таблица=stat_faceoff cтолбец=faceoff_home)
    imagettftext($image, $s_f, 0, 115, 430, $grey, $font, $row_team_1['faceoff_home']);
    imagettftext($image, $s_f, 0, 560, 430, $grey, $font, $row_team_2['faceoff_home']);
// 3-вбрасывания в гостях (таблица=stat_faceoff cтолбец=faceoff_guest)
    imagettftext($image, $s_f, 0, 115, 476, $grey, $font, $row_team_1['faceoff_guest']);
    imagettftext($image, $s_f, 0, 560, 476, $grey, $font, $row_team_2['faceoff_guest']);
// 4-выигранные вбрасывания (таблица=stat_faceoff cтолбец=faceoff_perc_wins_total)
    imagettftext($image, $s_f, 0, 115, 523, $grey, $font, $row_team_1['faceoff_perc_wins_total']);
    imagettftext($image, $s_f, 0, 560, 523, $grey, $font, $row_team_2['faceoff_perc_wins_total']);
// 5-выигранные вбрасывания дома (таблица=stat_faceoff cтолбец=faceoff_perc_wins_home)
    imagettftext($image, $s_f, 0, 115, 570, $grey, $font, $row_team_1['faceoff_perc_wins_home']);
    imagettftext($image, $s_f, 0, 560, 570, $grey, $font, $row_team_2['faceoff_perc_wins_home']);
// 6-выигранные вбрасывания в гостях (таблица=stat_faceoff cтолбец=faceoff_perc_wins_guest)
    imagettftext($image, $s_f, 0, 115, 617, $grey, $font, $row_team_1['faceoff_perc_wins_guest']);
    imagettftext($image, $s_f, 0, 560, 617, $grey, $font, $row_team_2['faceoff_perc_wins_guest']);
    
imagepng($image,'../template_img/new/match_'.$number_match.'_temp_9.png',9);
