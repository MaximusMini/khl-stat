...
<head>
    ...
    <title>Шаблон 5 (заброшенные шайбы) </title>
</head>
...
$id_connect_DB = new mysqli('localhost', 'root', '', 'db_preview');
    if($id_connect_DB){
        // формируем и выполняем запрос к таблице table_conf 
            $query_team_1 = 'SELECT * FROM stat_puck WHERE id_team='.$id_team_1;
            $data_team_1 = $id_connect_DB->query($query_team_1);
            $query_team_2 = 'SELECT * FROM stat_puck WHERE id_team='.$id_team_2;
...
$image = imagecreatefrompng('../template_img/temp_4.png');
...
// 1-заброшенные шайбы (таблица=stat_puck cтолбец=throw_puck)
    imagettftext($image, $s_f, 0, 115, 380, $grey, $font, $row_team_1['throw_puck']);
    imagettftext($image, $s_f, 0, 560, 380, $grey, $font, $row_team_2['throw_puck']);
// 2-заброшенные шайбы дома (таблица=stat_puck cтолбец=clear_loss_home)
    imagettftext($image, $s_f, 0, 115, 430, $grey, $font, $row_team_1['throw_puck_home']);
    imagettftext($image, $s_f, 0, 560, 430, $grey, $font, $row_team_2['throw_puck_home']);
// 3-заброшенные шайбы в гостях (таблица=stat_puck cтолбец=clear_loss_guest)
    imagettftext($image, $s_f, 0, 115, 476, $grey, $font, $row_team_1['throw_puck_guest']);
    imagettftext($image, $s_f, 0, 560, 476, $grey, $font, $row_team_2['throw_puck_guest']);
// 4-в среднем за матч (таблица=stat_puck cтолбец=ot_loss)
    imagettftext($image, $s_f, 0, 115, 523, $grey, $font, $row_team_1['throw_puck_average']);
    imagettftext($image, $s_f, 0, 560, 523, $grey, $font, $row_team_2['throw_puck_average']);
// 5-в среднем за матч дома (таблица=stat_puck cтолбец=ot_loss_home)
    imagettftext($image, $s_f, 0, 115, 570, $grey, $font, $row_team_1['throw_puck_average_home']);
    imagettftext($image, $s_f, 0, 560, 570, $grey, $font, $row_team_2['throw_puck_average_home']);
// 6-в среднем за матч в гостях (таблица=stat_puck cтолбец=ot_loss_guest)
    imagettftext($image, $s_f, 0, 115, 617, $grey, $font, $row_team_1['throw_puck_average_guest']);
    imagettftext($image, $s_f, 0, 560, 617, $grey, $font, $row_team_2['throw_puck_average_guest']);
    
imagepng($image,'../template_img/new/match_'.$number_match.'_temp_4.png',9);
