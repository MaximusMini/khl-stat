...
<head>
    ...
    <title>Шаблон 9 (броски соперника) </title>
</head>
...
$id_connect_DB = new mysqli('localhost', 'root', '', 'db_preview');
    if($id_connect_DB){
        // формируем и выполняем запрос к таблице table_conf 
            $query_team_1 = 'SELECT * FROM stat_throw_rival WHERE id_team='.$id_team_1;
            $data_team_1 = $id_connect_DB->query($query_team_1);
            $query_team_2 = 'SELECT * FROM stat_throw_rival WHERE id_team='.$id_team_2;
...
$image = imagecreatefrompng('../template_img/temp_8.png');
...
// 1-броски соперника (таблица=stat_throw_rival cтолбец=throw_rival)
    imagettftext($image, $s_f, 0, 105, 380, $grey, $font, $row_team_1['throw_rival']);
    imagettftext($image, $s_f, 0, 550, 380, $grey, $font, $row_team_2['throw_rival']);
// 2-броски соперника дома (таблица=stat_throw_rival cтолбец=throw_rival_home)
    imagettftext($image, $s_f, 0, 105, 430, $grey, $font, $row_team_1['throw_rival_home']);
    imagettftext($image, $s_f, 0, 550, 430, $grey, $font, $row_team_2['throw_rival_home']);
// 3-броски соперника в гостях (таблица=stat_throw_rival cтолбец=throw_rival_guest)
    imagettftext($image, $s_f, 0, 105, 476, $grey, $font, $row_team_1['throw_rival_guest']);
    imagettftext($image, $s_f, 0, 550, 476, $grey, $font, $row_team_2['throw_rival_guest']);
// 4-в среднем за матч (таблица=stat_throw_rival cтолбец=throw_rival_average)
    imagettftext($image, $s_f, 0, 105, 523, $grey, $font, $row_team_1['throw_rival_average']);
    imagettftext($image, $s_f, 0, 550, 523, $grey, $font, $row_team_2['throw_rival_average']);
// 5-в среднем за матч дома (таблица=stat_throw_rival cтолбец=throw_rival_average_home)
    imagettftext($image, $s_f, 0, 105, 570, $grey, $font, $row_team_1['throw_rival_average_home']);
    imagettftext($image, $s_f, 0, 550, 570, $grey, $font, $row_team_2['throw_rival_average_home']);
// 6-в среднем за матч в гостях (таблица=stat_throw_rival cтолбец=throw_rival_average_guest)
    imagettftext($image, $s_f, 0, 105, 617, $grey, $font, $row_team_1['throw_rival_average_guest']);
    imagettftext($image, $s_f, 0, 550, 617, $grey, $font, $row_team_2['throw_rival_average_guest']);
    
imagepng($image,'../template_img/new/match_'.$number_match.'_temp_8.png',9);
