...
<head>
    ...
    <title>Шаблон 8 (броски по воротам) </title>
</head>
...
$id_connect_DB = new mysqli('localhost', 'root', '07011989', 'db_preview');
    if($id_connect_DB){
        // формируем и выполняем запрос к таблице table_conf 
            $query_team_1 = 'SELECT * FROM stat_throw WHERE id_team='.$id_team_1;
            $data_team_1 = $id_connect_DB->query($query_team_1);
            $query_team_2 = 'SELECT * FROM stat_throw WHERE id_team='.$id_team_2;
...
$image = imagecreatefrompng('../template_img/temp_7.png');
...
// 1-броски по воротам (таблица=stat_puck cтолбец=throw_puck)
    imagettftext($image, $s_f, 0, 105, 380, $grey, $font, $row_team_1['total_throw']);
    imagettftext($image, $s_f, 0, 550, 380, $grey, $font, $row_team_2['total_throw']);
// 2-броски по воротам дома (таблица=stat_puck cтолбец=clear_loss_home)
    imagettftext($image, $s_f, 0, 105, 430, $grey, $font, $row_team_1['total_throw_home']);
    imagettftext($image, $s_f, 0, 550, 430, $grey, $font, $row_team_2['total_throw_home']);
// 3-броски по воротам в гостях (таблица=stat_puck cтолбец=clear_loss_guest)
    imagettftext($image, $s_f, 0, 105, 476, $grey, $font, $row_team_1['total_throw_guest']);
    imagettftext($image, $s_f, 0, 550, 476, $grey, $font, $row_team_2['total_throw_guest']);
// 4-в среднем за матч (таблица=stat_puck cтолбец=ot_loss)
    imagettftext($image, $s_f, 0, 105, 523, $grey, $font, $row_team_1['total_throw_average']);
    imagettftext($image, $s_f, 0, 550, 523, $grey, $font, $row_team_2['total_throw_average']);
// 5-в среднем за матч дома (таблица=stat_puck cтолбец=ot_loss_home)
    imagettftext($image, $s_f, 0, 105, 570, $grey, $font, $row_team_1['total_throw_average_home']);
    imagettftext($image, $s_f, 0, 550, 570, $grey, $font, $row_team_2['total_throw_average_home']);
// 6-в среднем за матч в гостях (таблица=stat_puck cтолбец=ot_loss_guest)
    imagettftext($image, $s_f, 0, 105, 617, $grey, $font, $row_team_1['total_throw_average_guest']);
    imagettftext($image, $s_f, 0, 550, 617, $grey, $font, $row_team_2['total_throw_average_guest']);
    
imagepng($image,'../template_img/new/match_'.$number_match.'_temp_7.png',9);
