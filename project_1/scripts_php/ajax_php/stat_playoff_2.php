...
// заброшенные шайбы
    imagettftext($image, 15, 0, 120, 385, $yellow, $font, $t1_arr['throw_puck']);
    imagettftext($image, 15, 0, 560, 385, $yellow, $font, $t2_arr['throw_puck']);
// пропущенные шайбы
    imagettftext($image, 15, 0, 120, 432, $yellow, $font, $t1_arr['allow_puck']);
    imagettftext($image, 15, 0, 560, 432, $yellow, $font, $t2_arr['allow_puck']);
// штрафное время
    imagettftext($image, 15, 0, 120, 478, $yellow, $font, $t1_arr['penalty_time']);
    imagettftext($image, 15, 0, 560, 478, $yellow, $font, $t2_arr['penalty_time']);
// броски по воротам
    imagettftext($image, 15, 0, 120, 525, $yellow, $font, $t1_arr['total_throw']);
    imagettftext($image, 15, 0, 560, 525, $yellow, $font, $t2_arr['total_throw']);
// выигранные вбрасывания
    imagettftext($image, 15, 0, 105, 571, $yellow, $font, $t1_arr['faceoff_perc_wins_total']);
    imagettftext($image, 12, 0, 155, 560, $yellow, $font_Xolonium, '%');
    imagettftext($image, 15, 0, 545, 571, $yellow, $font, $t2_arr['faceoff_perc_wins_total']);
    imagettftext($image, 12, 0, 595, 560, $yellow, $font_Xolonium, '%');
//// поражения ОТ
...
// сохраняем изображение
    imagepng($image,'../../template_img/new/playoff_'.$number_match.'_temp_p2.png',9);
